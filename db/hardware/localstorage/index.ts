...
const LocalStorageDriver: DbHardwareType = {
    ...
    getInklings: async function (journalId: string): Promise<Inklings> {
        try {
            return (
                JSON.parse(localStorage.getItem(genInklingKey(journalId))) || []
            );
        } catch (err) {
            // No Inklings
            return [];
    ...
    commitInklings: async function (
        ...
        localStorage.setItem(
            genInklingKey(journalId),
            JSON.stringify(inklings)
        );
    },
    ...
    getJournal: async function (journalId: string): Promise<Journal> {
        try {
            return (
                JSON.parse(localStorage.getItem(genJournalKey(journalId))) || []
            );
        } catch (err) {
            // No Journal
            return [];
    ...
    deleteJournal: async function (journalId: string): Promise<void> {
        ...
        localStorage.setItem(
            genJournalIdsKey(),
            JSON.stringify(journalIdsToKeep)
        );
    ...
    getJournalMetadata: async function (
        ...
        try {
            return JSON.parse(
                localStorage.getItem(genJournalMetadataKey(journalId))
            );
        } catch (err) {
            return null;
        }
    ...
    getCurrentIdentityIds: async function (
        journalId: string
    ): Promise<string[]> {
        try {
            const journal: Journal = await LocalStorageDriver.getJournal(
                journalId
            );

            const lastReflections: Reflection[] =
                journal.length > 0
                    ? journal[journal.length - 1].reflections
                    : [];

            return lastReflections
                .filter(
                    ({ data }: Reflection) => data === ReflectionDecision.Keep
                )
    ...
    // THOUGHTS
    _convertInklingsToThoughts: async function (
        journalId: string,
        time: number
    ) {
        // 1. Get Inklings
        const inklings: Inklings = await LocalStorageDriver.getInklings(
            journalId
        );
    ...
    _commitThoughts: async function (journalId: string, thoughts: Thought[]) {
        // 0. Get ThoughtsDict
        const thoughtsDict: ThoughtsDict =
            await LocalStorageDriver.getThoughtsDict(journalId);
    ...
    getThoughtsDict: async function (journalId: string): Promise<ThoughtsDict> {
        try {
            return (
                JSON.parse(
                    localStorage.getItem(genThoughtsDictKey(journalId))
                ) || {}
            );
        } catch (err) {
            return {};
